...
@bot.command(brief="*UNDER MAINTENANCE*")
...
@bot.command(brief="weather(zipcode): uses Weather API to tell the weather at the ZIP Code")
...
@bot.command(brief="yodaTranslate(phrase): Garbles a phrase into something Yoda would say. (please put your phrase into quotes)")
...
#async def art(ctx):
  #variable to hold the url
#  my_secret_art = os.environ['ArtKey']
  #url="https://api.hotpot.ai/" + zip + ",us&appid=" + my_secret_weather
  #req=requests.get(url)
  #data variable that holds the json data that the api holds
  #data = req.json()
...
@bot.command(aliases=["rockpaperscissors","rockPaperScissors","rps"],brief="RPS(rock, paper, or scissors): it's Rock Paper Scissors, that's it.")
...
#comedy gold
@bot.command(brief="Givves the best puns known to man.")
async def puns(ctx):
  global num
  global time
  if num == 0:
